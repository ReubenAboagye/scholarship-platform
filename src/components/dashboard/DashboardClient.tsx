...
export default function DashboardClient({
  ...
  return (
    ...
    >
      ...
      <motion.div variants={item} className="flex items-center justify-between gap-4 pt-1">
        <div>
        ...
        <Link
          href="/scholarships"
          className="flex-shrink-0 flex items-center gap-1.5 text-xs font-bold text-slate-600 bg-white border border-slate-200 hover:border-slate-300 hover:bg-slate-50 transition-all px-4 py-2 rounded-xl group"
        >
        ...
        {stats.map((s) => {
          ...
          return (
            <motion.div variants={item} key={s.label}>
              <Link
                href={s.href}
                className="flex items-center gap-3 bg-white border border-slate-200 hover:border-slate-300 px-4 py-3 rounded-xl transition-all group"
              >
      ...
      {/* ── AI Match CTA ── */}
      <motion.div variants={item} className="flex items-center justify-between gap-4 bg-white border border-slate-200 rounded-xl px-5 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-slate-900">
              {profileComplete ? "Run AI matching" : "Complete your profile to unlock AI matching"}
            </p>
            <p className="text-xs text-slate-400 mt-0.5">
              {profileComplete
                ? "Get a ranked list of scholarships tailored to your profile."
                : "Add your degree, field of study and country to get started."}
            </p>
          </div>
        </div>
        <Link
          href={profileComplete ? "/dashboard/match" : bannerHref}
          className="flex-shrink-0 flex items-center gap-1.5 text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 px-4 py-2 rounded-lg transition-all group"
        >
          {profileComplete ? "Match now" : "Set up"} <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-0.5 transition-transform" />
        </Link>
      </motion.div>
    </motion.div>
  );
