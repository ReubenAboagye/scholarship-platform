    ...
    <motion.div variants={stagger} initial="hidden" animate="show" className="max-w-5xl mx-auto space-y-5 pb-10">
      ...
      <motion.div variants={stagger} className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {[
          ...
          <motion.div variants={fade} key={s.label}>
            <Link href={s.href} className="flex items-center gap-3 bg-white border border-slate-200 hover:border-slate-300 px-3 py-3 rounded-xl transition-all group">
      ...
      {dueThisWeek.length > 0 && (
        <motion.div variants={fade}>
          <div className="flex items-center justify-between mb-2">
            ...
            <Link href="/dashboard/tracker" className="text-xs font-semibold text-brand-600 hover:text-brand-700">View all →</Link>
          </div>
      ...
      <motion.div variants={fade}>
        <div className="flex items-center justify-between mb-2">
          ...
          <Link href={hasMatchHistory ? "/dashboard/matches" : "/dashboard/match"}
        ...
        {topMatches.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-xl p-6 text-center">
            ...
            <Link href={profileComplete ? "/dashboard/match" : bannerHref}
      ...
      <motion.div variants={fade}>
        <div className="flex items-center justify-between mb-2">
          ...
          <Link href="/dashboard/tracker" className="text-xs font-semibold text-brand-600 hover:text-brand-700">Manage →</Link>
        </div>

        {tracked.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-xl p-5 text-center">
            <p className="text-sm font-semibold text-slate-500">No applications tracked</p>
            <Link href="/scholarships" className="text-xs text-brand-600 hover:underline mt-1 inline-block">
